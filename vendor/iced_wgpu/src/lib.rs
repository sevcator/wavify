...
impl Renderer {
    ...
    fn prepare(
        ...
        // Wavify patch: `render` skips a layer that is off the window or
        // under a pixel in size, and indexes every pipeline's prepared
        // layers by counting the layers it draws. Preparing such a layer
        // anyway put every later layer one behind: an overlay drew another
        // layer's text in its own clip, i.e. its text went missing. The same
        // skip here keeps the two in step.
